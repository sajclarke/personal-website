        ...
        <div className="flex w-full md:w-1/2 justify-center md:justify-start text-white font-semibold">
          <Link href='/'>
            <div className="flex text-gray-600  cursor-pointer">
              <span className="text-xl pl-2"><i className="em em-grinning"></i> Shannon Clarke</span>
            </div>
          </Link>
        </div>
          ...
          <ul className="list-reset">
            ...
            {['About'].map((item, index) => (
              <li key={index} className="mr-1">
                <Link href={`/${item.toLowerCase()}`}><a className="inline-block text-gray-600 hover:bg-indigo-100 hover:text-indigo-500  rounded py-2 px-4">{item}</a></Link>
              </li>
            ))}
